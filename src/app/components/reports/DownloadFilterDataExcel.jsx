...
// console.log("data",data);
    ...
    // Flatten serviceCenters to handle pincodeSupported
    const formattedServiceCenters = serviceCenters.map(sc => ({
      ...sc,
      pincodeSupported: sc.pincodeSupported?.join(", ") || ""  // convert array → string
    }));

    // Convert the data to worksheets
    const customerSheet = utils.json_to_sheet(customers);
    const serviceCenterSheet = utils.json_to_sheet(formattedServiceCenters);
    const technicianSheet = utils.json_to_sheet(technicians);
    const brandSheet = utils.json_to_sheet(brands);
