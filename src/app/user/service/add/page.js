...
const {user}=useUser()
    ...
    const Regiter = async (reqdata) => {
        try {
            const reqData2 = user?.user?.role === "ADMIN" ? reqdata : { ...reqdata, brandId: user?.user?._id, brandName: user?.user?.brandName }
            setLoading(true)
            let response = await http_request.post(`/serviceRegistration`, reqData2)
            const { data } = response
