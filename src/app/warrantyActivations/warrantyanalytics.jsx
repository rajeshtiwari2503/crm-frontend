...
export default function WarrantyAnalytics() {
    ...
    useEffect(() => {
        if (user && user.user.role === "BRAND") {
            setSelectedBrand(user.user.brandName);
        }
    }, [user]);
    useEffect(() => {
        if (!user) return;

        // For Admin, selectedBrand can be empty (dropdown controlled)
        if (user.user.role === "ADMIN" || (user.user.role === "BRAND")) {
            const fetchAnalytics = async () => {
                try {
                    setLoading(true);

                    // const url = user.user.role === "ADMIN" && selectedBrand
                    //   ?  `/warranty-analyticsByBrand?brand=${encodeURIComponent(selectedBrand)}`
                    //   : user.user.role === "ADMIN"?"/warranty-analytics" 
                    //   :`/warranty-analyticsByBrand?brand=${encodeURIComponent(selectedBrand)}`;
                    let url = "";

                    if (selectedBrand && selectedBrand !== "") {
                        url = `/warranty-analyticsByBrand?brand=${encodeURIComponent(selectedBrand)}`;
                    } else if (user.user.role === "ADMIN") {
                        url = "/warranty-analytics";
                    } else {
                        url = `/warranty-analyticsByBrand?brand=${encodeURIComponent(user.user.brandName)}`;
                    }
    ...
    return (
        <div className="max-w-7xl mx-auto px-4  ">
            <div className="grid grid-cols-1 md:grid-cols-2 mb-5 mt-5 items-center">
