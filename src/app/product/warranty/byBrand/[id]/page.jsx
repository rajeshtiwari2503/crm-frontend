...
const ProductWarrantyPage = ({ params }) => {
 ...
 const [refresh, setRefresh] = useState("")
  ...
  }, [page, rowsPerPage, user,refresh]); // Fetch data when page or rowsPerPage changes
  useEffect(() => {
    getAllProduct()
    getAllBrand()
    fetchProductWarranty();
  ...
  const fetchProductWarranty = async () => {
    ...
    const reqData =  `/getAllProductWarrantyByIdWithPage/${params.id}?page=${page + 1}&limit=${rowsPerPage}`  
    try {
      setLoading(true);
